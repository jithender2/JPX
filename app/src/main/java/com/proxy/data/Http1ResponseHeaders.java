...
public class Http1ResponseHeaders extends Http1Headers implements HttpResponseHeaders, Serializable {
	...
	public List<NameValue> cookieValues() {
		// Get the Set-Cookie headers and map each one to a NameValue object
		return getHeaders("Set-Cookie").stream().map(CookieUtils::parseCookieHeader) // Replace with a direct call to a utility method
